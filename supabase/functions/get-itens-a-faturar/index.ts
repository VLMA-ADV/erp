...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para visualizar itens a faturar" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!dataInicio || !dataFim) {
      return new Response(JSON.stringify({ error: "Informe data_inicio e data_fim" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data, error } = await supabase.rpc("get_itens_a_faturar", {
      p_user_id: user.id,
