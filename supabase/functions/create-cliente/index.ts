...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clienteId && Array.isArray(body.responsaveis_financeiros)) {
      try {
        await syncResponsaveisFinanceiros(supabase, clienteId, body.responsaveis_financeiros);
      } catch (rfErr) {
        console.error("Erro ao sincronizar responsáveis financeiros:", rfErr);
        return new Response(
          JSON.stringify({
            error: "Cliente criado, mas falhou ao salvar responsáveis financeiros",
