...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.nome || !Array.isArray(body.itens)) {
      return new Response(JSON.stringify({ error: "Campos obrigatórios: nome e itens" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para salvar tabela de preço" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data, error } = await supabase.rpc("upsert_tabela_preco", {
      p_user_id: user.id,
      p_tabela_id: body.id ?? null,
      p_nome: body.nome,
      p_itens: body.itens,
    });
