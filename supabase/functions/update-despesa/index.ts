...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para editar despesas" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!body.id) {
      return new Response(JSON.stringify({ error: "ID da despesa é obrigatório" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data, error } = await supabase.rpc("update_despesa", {
      p_user_id: user.id,
