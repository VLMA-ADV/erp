  ...
  try {
    ...
    const hasPermission = permissionsData?.some(
      (p: any) =>
        ...
        p.permission_key === "finance.faturamento.revert" ||
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para alterar etapa da revisão de fatura" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    const { data, error } = await supabase.rpc("set_revisao_fatura_status", {
      p_user_id: user.id,
      p_payload: body,
    })
