...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para excluir cards do CRM" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!id) {
      return new Response(JSON.stringify({ error: "ID do card é obrigatório" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data, error } = await supabase.rpc("delete_crm_pipeline_card", {
      p_user_id: user.id,
      p_card_id: id,
    });
