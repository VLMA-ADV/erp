...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get roles usando função RPC
    const { data: roles, error: rolesError } = await supabase
      .rpc('get_roles', { p_user_id: user.id });

    if (rolesError) {
      console.error("Error fetching roles:", rolesError);
      return new Response(
        JSON.stringify({ 
          error: rolesError.message || "Error fetching roles",
