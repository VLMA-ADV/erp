...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const hasPermission = permissionsData?.some((p: any) =>
      p.permission_key === "crm.pipeline.read" ||
    ...
    if (!hasPermission) {
      return new Response(JSON.stringify({ error: "Você não tem permissão para visualizar anexos do CRM" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!anexoId) {
      return new Response(JSON.stringify({ error: "ID do anexo é obrigatório" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data, error } = await supabase.rpc("get_crm_pipeline_card_anexo", {
      p_user_id: user.id,
      p_anexo_id: anexoId,
    });
