...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.caso_id || !body.nome || !body.arquivo_nome || !body.arquivo_base64) {
      return new Response(JSON.stringify({ error: "Campos obrigatórios: caso_id, nome, arquivo_nome, arquivo_base64" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data, error } = await supabase.rpc("create_caso_anexo", {
      p_user_id: user.id,
