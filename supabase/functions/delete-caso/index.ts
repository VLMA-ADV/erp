  ...
  try {
    ...
    const hasPermission = permissionsData?.some(
      (p: { permission_key: string }) =>
        ...
        p.permission_key === "contracts.casos.*" ||
    ...
    if (!hasPermission) {
      return new Response(
        JSON.stringify({ error: "Você não tem permissão para excluir caso" }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const { data, error } = await supabase.rpc("delete_caso_draft", {
      p_user_id: user.id,
      p_caso_id: body.id,
    });
    ...
    if (tenantUser?.tenant_id) {
      await createAuditLog({
        ...
        acao: "delete",
        userId: user.id,
        dadosNovos: { id: body.id, deleted: true },
