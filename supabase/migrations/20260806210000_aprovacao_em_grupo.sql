...
BEGIN
  ...
  IF v_action = 'avancar' THEN
    IF v_old_status = 'em_revisao' THEN
      ...
      -- Aprovar/revisar um item do grupo leva os demais junto (Filipe, 06/08:
      -- "aprovar o grupo aprova os dois automaticamente"). Sem isso o
      -- agrupamento seria so visual.
      WHERE bi.id = v_item.id
         OR (v_item.grupo_id IS NOT NULL
             AND bi.grupo_id = v_item.grupo_id
             AND bi.tenant_id = v_item.tenant_id);
  ...
  ELSE
    ...
    -- devolver para revisao tambem arrasta o grupo inteiro
    WHERE bi.id = v_item.id
       OR (v_item.grupo_id IS NOT NULL
           AND bi.grupo_id = v_item.grupo_id
           AND bi.tenant_id = v_item.tenant_id);
