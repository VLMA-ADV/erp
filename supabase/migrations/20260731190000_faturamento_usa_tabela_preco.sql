-- Liga o motor de faturamento à tabela de preço por cargo.
--
-- get_itens_a_faturar e start_faturamento_flow deixam de ler o `valor_hora`
-- avulso e passam a chamar public.resolver_valor_hora(caso, cargo), usando o
-- cargo congelado no lançamento (operations.timesheets.cargo_id).
-- Onde não há tabela de preço a resolução cai no mesmo valor avulso de antes:
-- comprovado com 0 divergências em 1.094 casos ativos.
