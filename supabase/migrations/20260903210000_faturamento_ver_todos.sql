-- =====================================================================
-- Ver o faturamento inteiro sem estar na area VLMA/Financeiro
--
-- Ate aqui, ver tudo dependia do CENTRO DE CUSTO: so area 'VLMA', 'Financeiro',
-- sem area ou super-admin. Socio de area ve a propria area — e isso e
-- proposital, esta escrito no proprio codigo.
--
-- A Bruna Bogucheski e socia e coordenadora, mas da area 'Contratos', entao via
-- so o que era dela. Nao era bug.
-- =====================================================================
...
BEGIN
  ...
  v_can_view_all :=
    ...
    OR EXISTS (
      ...
      WHERE p.permission_key = 'finance.faturamento.ver_todos'
    );
...
SELECT DISTINCT t.tenant_id, 'finance.faturamento.ver_todos',
       'Ver o faturamento de todo o escritorio, independente do centro de custo'
FROM core.tenant_users t
WHERE t.tenant_id = 'd51463dd-a6b3-40e7-9488-854eba80a210'
  AND NOT EXISTS (SELECT 1 FROM core.permissions p
                   WHERE p.tenant_id = t.tenant_id AND p.chave = 'finance.faturamento.ver_todos');
...
WHERE c.tenant_id = 'd51463dd-a6b3-40e7-9488-854eba80a210'
  AND u.email = 'bruna.bogucheski@vlma.com.br'
