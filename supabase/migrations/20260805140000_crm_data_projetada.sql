-- CRM4 · Data projetada e projecao por etapa (pedido Filipe 04/08).
--
-- Dois campos novos no card:
--   data_projetada          — quando se espera fechar a oportunidade
--   valor_faturado_prox_mes — campo pedido pelo cliente para a etapa Conversao
--
-- valor_faturado_prox_mes entra com DEFAULT 0 e NOT NULL, no mesmo padrao dos
-- outros valores do card, para os 71 cards que ja existem nao ficarem nulos e
-- quebrarem soma na tela. data_projetada fica nula: "sem data" e uma resposta
-- legitima, e chutar uma data seria pior que deixar vazio.

ALTER TABLE crm.pipeline_cards
  ADD COLUMN IF NOT EXISTS data_projetada date,
  ADD COLUMN IF NOT EXISTS valor_faturado_prox_mes numeric(14,2) NOT NULL DEFAULT 0;
