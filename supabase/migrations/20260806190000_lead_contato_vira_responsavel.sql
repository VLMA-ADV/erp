CREATE OR REPLACE FUNCTION public.converter_lead_em_cliente(p_user_id uuid, p_card_id uuid, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- O "contato" do lead e a pessoa dentro do cliente. Filipe confirmou em 06/08
  -- que e o mesmo papel do responsavel financeiro do cadastro, entao na
  -- conversao ele passa a ser gravado la em vez de ficar so no card.
  IF v_card.lead_contato IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM crm.clientes_responsaveis_financeiros rf
     WHERE rf.cliente_id = v_cliente_id
       AND lower(trim(rf.nome)) = lower(trim(v_card.lead_contato))
  ) THEN
    INSERT INTO crm.clientes_responsaveis_financeiros (cliente_id, nome, email)
    VALUES (v_cliente_id, v_card.lead_contato, v_card.lead_email);
  END IF;
