-- Report Builder v2 — bugfix + filtros do daily Filipe 02/06.
--
-- Bug 1 (BLOQUEADOR): a versão atual referencia `finance.grupos_impostos`,
-- mas a tabela vive em `contracts.grupos_impostos`. Resultado: a query EXECUTE
-- estoura com 42P01 "relation does not exist" toda vez que o usuário tenta
-- "Gerar Preview" em Contratos. Filipe identificou isso na call 02/06.
--
-- Bug 2: schema `public` de search_path engole o resolvedor; precisa
-- listar explicitamente `contracts, crm, finance, people, operations`.
--
-- Daily 02/06 — pediu:
--  • Busca textual livre (procurar "auditoria do cliente X")
--  • Filtros dedicados: cliente_id, responsavel_id, regime_fiscal
--  • Mais colunas em contratos (auditoria completa)
