-- Expoe recorrencia_id/parcela_numero nas linhas do fluxo, para a tela poder
-- filtrar "so as recorrentes". Pedido do Filipe em 25/08, junto com o parcelado:
-- de nada adianta poder editar a serie inteira se achar uma linha de serie no
-- meio do mes depende de sorte.
  ...
  SELECT jsonb_build_object(
    ...
    'pagar', COALESCE((SELECT jsonb_agg(to_jsonb(x) ORDER BY x.vencimento, x.descricao) FROM (
        SELECT id, descricao, fornecedor_nome, empresa_nome, conta_codigo, plano_grupo, centro_nome,
               ...
               -- Filipe, 25/08: "precisamos de algum filtro no painel para buscar
               -- as despesa recorrente para editar a serie ou individualizada".
               -- Sem isto a tela nao tem como saber quais linhas sao de serie.
               recorrencia_id, parcela_numero
