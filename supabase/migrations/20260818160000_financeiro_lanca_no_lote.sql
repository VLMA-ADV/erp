-- =====================================================================
-- O financeiro lança e enxerga despesa dentro do lote de outra pessoa
--
-- Filipe, 17/08: "quero que replique a estrutura do lançamento da despesa
-- nesta tela, de modo que o financeiro não precise sair do módulo para fazer
-- essa função".
--
-- Duas travas impediam isso, e as duas fazem sentido para o caso normal:
--   1. validar_lote_da_despesa recusava lote de outra pessoa;
--   2. get_despesas mostra a despesa para quem a lançou (created_by), então a
--      despesa que a Jessika digitasse no lote da Aline sumiria da tela da
--      Aline — o dono do próprio lote deixaria de ver o próprio gasto.
--
-- A primeira abre só para quem tem finance.contas_pagar.write (o financeiro),
-- não para todo mundo. A segunda passa a mostrar também o que está num lote
-- DA PESSOA, independente de quem digitou — que é o certo: o dinheiro é dela,
-- o acerto vai para o nome dela.
-- =====================================================================
...
-- get_despesas: dono do lote continua vendo o gasto do proprio lote
