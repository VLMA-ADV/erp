-- RF-081 / Z-7 — polo (ativo|passivo) em contracts.casos quando natureza é contencioso.
-- Pré-validação: `natureza_caso` não existe como coluna; vem em `regras_financeiras` (JSONB).
-- Coluna `polo` fica em contracts.casos (nível caso), alinhada ao pedido RF-081 sem refatorar JSON.

ALTER TABLE contracts.casos
  ADD COLUMN IF NOT EXISTS polo varchar(10) NULL;

ALTER TABLE contracts.casos
  DROP CONSTRAINT IF EXISTS casos_polo_chk;

ALTER TABLE contracts.casos
  ADD CONSTRAINT casos_polo_chk CHECK (polo IS NULL OR polo IN ('ativo', 'passivo'));
