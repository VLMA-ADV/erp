-- =====================================================================
-- Hora em caso com regra mensal nao e cobrada de novo
--
-- Filipe, 03/09: "alterei a regra de cobranca para mensal de 25k mas ele ainda
-- segue cobrando por hora os lancamentos... da uma diferenca de 125k".
--
-- Caso 319 (PBIM Juridico): mensalidade de R$ 25.000 + R$ 116.921,80 em 127
-- lancamentos de hora. O caso cobrava as duas coisas.
--
-- Causa: a hora sempre valia horas x valor_hora, sem olhar a regra do caso. A
-- tabela de preco por cargo continua existindo depois que o caso vira mensal,
-- entao a hora seguia ganhando valor — e a mensalidade ja paga o trabalho.
--
-- A hora continua APARECENDO: e o registro do que foi feito e vai no relatorio
-- do cliente. So deixa de somar dinheiro.
-- =====================================================================
  ...
  RETURN (
    WITH base_timesheet AS (
      SELECT
        ...
        (
          -- Caso com regra MENSAL (ou mensalidade de processo / carteira /
          -- salario minimo) ja cobra pelo periodo: a hora entra pelo registro,
          -- valendo zero. Antes ela era valorizada pela tabela de preco mesmo
          -- assim, e o caso cobrava as duas coisas — Filipe, 03/09, caso 319:
          -- mensalidade de R$ 25.000 somando com R$ 116.921,80 de horas.
          --
          -- 'cobra_excedente' = true e a excecao deliberada: o combinado inclui
          -- N horas e o que passar disso se cobra. Ai a hora volta a valer.
          CASE
            ...
            THEN 0
            ELSE COALESCE(t.horas, 0) * COALESCE(public.resolver_valor_hora(cs.id, t.cargo_id), 0)
          END
      ...
      -- Regra vigente do caso, para saber se a hora e cobravel. Precisa vir
      -- DEPOIS do join de casos: LATERAL so enxerga o que ja esta no FROM.
      CROSS JOIN LATERAL (
        SELECT
