-- =====================================================================
-- O revisor configurado no caso manda no faturamento
--
-- Filipe, 02/09: trocou o revisor de quatro casos para a Bruna Fedatto e o
-- faturamento seguia mostrando Tiago e Amanda. "Pode alterar tudo, desde o
-- comeco... reescrever e recomecar pra nova revisora... e pode manter essa
-- regra como fixa daqui pra frente."
--
-- Por que so parte aparecia certa: o nome saia de
--   COALESCE(gravado no item, configurado no caso, automatico)
-- e o gravado no item so existe depois que ALGUEM AGE sobre ele. Entao item
-- ainda em revisao ja mostrava a Bruna; item que ja passara pela revisao ficava
-- com quem tinha revisado.
--
-- Agora o caso vem primeiro. Duas consequencias tratadas aqui:
--   1. quem de fato revisou nao se perde — continua em
--      finance.revisao_fatura_itens_historico (688 registros, intacta);
--   2. o campo gravado tambem dava ACESSO ao item. Mudar so o rotulo criaria
--      uma revisora que aparece como responsavel e nao consegue abrir o item.
--      Por isso a visibilidade acompanha.
-- =====================================================================
  ...
  RETURN (
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          -- O revisor configurado no CASO manda, mesmo em item ja revisado (Filipe,
      -- 02/09: "reescrever e recomecar pra nova revisora... pode manter essa
      -- regra como fixa daqui pra frente"). Antes o gravado no item vinha na
      -- frente, entao trocar o revisor do caso nao refletia no que ja passara
      -- pela revisao.
      --
      -- Quem de fato revisou nao se perde: fica em
      -- finance.revisao_fatura_itens_historico, que esta intacta.
      'responsavel_revisao_nome', COALESCE(rev_colab.nome, rev_actor_colab.nome, auto_rev.nome),
    ...
    WHERE bi.tenant_id = v_tenant_id
      ...
      AND (
        ...
        -- Se o nome exibido passa a vir do caso, o ACESSO tem de vir junto:
        -- senao a nova revisora aparece como responsavel e nao consegue abrir
        -- o item, enquanto o antigo continua vendo.
