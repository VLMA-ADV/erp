-- Conserta "Postergar" pra valer (pedido Filipe 11/08 e 12/08), e um bug mais
-- fundo que apareceu ao testar: o extrato de get_itens_a_faturar nunca devolvia
-- origem_id.
--
-- Ate aqui, postergar so gravava operations.timesheets.periodo_faturamento —
-- e nada no sistema lia essa coluna. O item nao saia da fatura atual (na
-- revisao, o billing_item continuava em_revisao) e nao aparecia na fatura do
-- mes escolhido (get_itens_a_faturar filtrava por data_lancamento, a data do
-- trabalho, nao por periodo_faturamento). O botao nao fazia o que dizia.
--
-- Ao testar de verdade (nao so ler o codigo), achei um segundo problema, mais
-- grave: o extrato que get_itens_a_faturar devolve para cada caso NUNCA incluia
-- 'origem_id' no jsonb de cada linha. O front (itens-a-faturar-list.tsx) usa
-- exatamente linha.origem_id para descobrir quais timesheets postergar ou
-- excluir — sem esse campo, o filtro `Boolean(linha.origem_id)` nunca achava
-- nada, e os dois botoes (Postergar e o Excluir que acabei de subir)
-- SEMPRE caiam no "este caso nao tem lancamentos... postergaveis/excluiveis",
-- mesmo com timesheets normais no caso. Um bug anterior a esse pedido, que so
-- apareceu porque desta vez testei o fluxo ponta a ponta antes de aplicar.
--
-- O conserto tem tres pontas:
--   1. get_itens_a_faturar passa a devolver origem_id em cada linha do extrato.
--   2. get_itens_a_faturar passa a filtrar pelo PERIODO EFETIVO — data_lancamento,
--      a menos que periodo_faturamento tenha sido definido, ai vale ele. Isso faz
--      o item aparecer na fila do mes escolhido.
--   3. Uma funcao unica, postergar_timesheet, faz as duas coisas do postergar
--      juntas: grava o periodo novo E cancela o billing_item atual (se ja
--      existir, na revisao) — senao o item continuaria contando na fatura
--      atual, com o mesmo trabalho contando duas vezes quando fosse pego de
--      novo no mes de destino.
--
-- data_referencia continua sendo a data real do trabalho (nao muda) — o que
-- muda e so o mes em que o item entra na fila de faturamento. O corpo de
-- get_itens_a_faturar abaixo e copia exata da versao em producao, so com essas
-- duas linhas trocadas (conferido por diff antes de aplicar).
...
CREATE OR REPLACE FUNCTION public.postergar_timesheet(p_user_id uuid, p_timesheet_id uuid, p_periodo date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  v_tenant_id uuid;
  v_bi finance.billing_items%ROWTYPE;
BEGIN
  ...
  IF p_periodo IS NULL THEN RAISE EXCEPTION 'Novo período é obrigatório'; END IF;

  IF NOT EXISTS (
    SELECT 1 FROM operations.timesheets t WHERE t.id = p_timesheet_id AND t.tenant_id = v_tenant_id
  ) THEN
    RAISE EXCEPTION 'Timesheet não encontrado';
  END IF;

  -- Se ja existe billing_item ativo para esse timesheet (postergar a partir da
  -- revisao, nao da fila pre-batch), ele precisa sair da fatura atual — senao
  -- o item continua contando agora E de novo quando for pego no mes de destino.
  SELECT * INTO v_bi FROM finance.billing_items bi
  WHERE bi.tenant_id = v_tenant_id AND bi.origem_tipo = 'timesheet'
    AND bi.origem_id = p_timesheet_id AND bi.status <> 'cancelado';

  IF FOUND THEN
    IF v_bi.status = 'faturado' THEN
  ...
  SET periodo_faturamento = p_periodo, updated_at = now(), updated_by = p_user_id
  WHERE id = p_timesheet_id AND tenant_id = v_tenant_id;

  RETURN jsonb_build_object('id', p_timesheet_id, 'periodo_faturamento', p_periodo);
END;
$function$;
