-- Item 5 PR1 (Filipe daily 28/04): get_contrato hoje retorna 'numero' (bigint
-- legacy) mas não 'numero_sequencial' (RF-064 canonical). Frontend e
-- breadcrumb querem 'numero_sequencial' pra render "Contrato N" consistente.
-- Mesma classe de polo/observacao/grupo_imposto: RPC esquece chave.
--
-- Aplicado em DEV via Cursor MCP em 2026-04-28. Smoke 4/4 verde:
--   1) tem_chave: true
--   2) regression check (resp_prosp/canal/polo/obs preservados): true
--   3) na_tabela=4→via_rpc="4"; 12→"12"; 15→"15" (3/3 match)
--   4) sigs: 1 (sem overload)
