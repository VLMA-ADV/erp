-- Painel de gestão de horas passa a usar o preço por senioridade.
--
-- Esta função ficou de fora da correção de 31/07 (PR #262): continuava lendo
-- o `valor_hora` avulso do caso e ignorando a tabela por cargo. Efeito: os 7
-- casos que cobram por senioridade apareciam valendo R$ 0,00 no painel — o
-- mesmo erro que já foi corrigido no faturamento.
--
-- Pré-requisito da aba "Gestão Geral": sem isto, a tela nova nasceria errada.
  ...
  RETURN (
    ...
    ts AS (
      SELECT
        ...
        -- Passa a usar o resolver: com tabela de preço por cargo, o valor
        -- depende de QUEM lançou. Antes lia só o valor avulso do caso, e os 7
        -- casos por senioridade apareciam a R$ 0,00 neste painel.
