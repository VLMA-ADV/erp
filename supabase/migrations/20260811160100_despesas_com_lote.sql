-- Despesa dentro de lote (ver 20260811160000_lotes_de_despesa.sql).
--
-- create/update passam a aceitar lote_id, sempre pela mesma regra: lote da
-- propria pessoa e ainda aberto. Despesa em lote nasce nao reembolsavel — o
-- adiantamento ja pagou, e o acerto sai uma vez so, no fechamento.
...
BEGIN
  ...
  INSERT INTO operations.despesas (
    ...
    -- Dentro de um lote o dinheiro ja foi adiantado: o acerto sai uma vez so,
    -- pelo saldo do lote, nunca por reembolso individual.
    CASE WHEN v_lote_id IS NOT NULL THEN false
         ELSE COALESCE((p_payload->>'reembolsavel')::boolean, true) END,
    v_lote_id,
    p_user_id, p_user_id
...
BEGIN
  ...
  SET
    ...
    reembolsavel = CASE
      WHEN (CASE WHEN v_troca_lote THEN v_lote_id ELSE d.lote_id END) IS NOT NULL THEN false
      WHEN p_payload ? 'reembolsavel' THEN COALESCE((p_payload->>'reembolsavel')::boolean, true)
      ELSE d.reembolsavel END,
    status = CASE
