-- Passo 7: liberar hora sem preço passa a ser RECUSADO, não faturado a zero.
--
-- Decisão do cliente (31/07): melhor travar e mandar arrumar o cadastro do que
-- deixar passar um lançamento que vira R$ 0,00 na fatura do cliente. Era assim
-- que os 6 casos com tabela de preço vinham saindo de graça, calados.
--
-- A recusa só vale para caso que COBRA POR HORA. Caso de projeto/mensalidade
-- registra hora para controle e legitimamente não gera valor por hora — esse
-- segue passando normalmente.
CREATE OR REPLACE FUNCTION public.validar_precos_antes_de_liberar(
  p_tenant_id uuid,
  p_caso_ids  uuid[]
)
RETURNS void
...
STABLE
SECURITY DEFINER
SET search_path TO 'public', 'contracts', 'operations', 'people'
AS $function$
DECLARE
  v_msg text;
BEGIN
  ...
  IF v_msg IS NOT NULL THEN
    RAISE EXCEPTION 'Não dá para liberar: sem valor/hora definido para %. Cadastre o valor na regra de cobrança do caso (ou o cargo na tabela de preço) antes de liberar.', v_msg;
  END IF;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.validar_precos_antes_de_liberar(uuid, uuid[]) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.validar_precos_antes_de_liberar(uuid, uuid[]) TO authenticated, service_role;

-- start_faturamento_flow chama a validação antes de criar itens.
