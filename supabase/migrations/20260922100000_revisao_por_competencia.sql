-- =====================================================================
-- Revisão de fatura por COMPETÊNCIA (1/3 da fila dentro da revisão)
--
-- Decisão do cliente (set/2026): a fila de liberação deixa de ser uma tela à
-- parte e passa a aparecer DENTRO da revisão, como etapa "Na fila", com abas
-- por competência — o mês em que se cobra (hora de agosto = competência
-- setembro = periodo_faturamento). Uma aba por mês com item pendente ou fila,
-- mais o mês seguinte.
--
-- Esta migração dá à revisão o que a tela precisa para montar as abas:
--
--  1. finance.escopo_faturamento(p_user_id): a regra de visibilidade que
--     estava embutida em get_revisao_fatura (vê tudo: '*', sem área, área
--     VLMA/Financeiro ou concessão NOMINAL finance.faturamento.ver_todos;
--     senão, escopo por centro de custo). Sai para uma função porque a fila
--     (get_fila_por_competencia, 20260922101000) precisa da MESMA regra — e
--     duas cópias da mesma regra é como a tela e o gerador divergiram em
--     31/08 e 03/09.
--
--  2. get_revisao_fatura ganha p_competencia (date, opcional, no fim) e cada
--     item passa a trazer periodo_inicio, periodo_fim e competencia
--     (= mês de periodo_inicio). Com p_competencia, filtra por esse mês; sem,
--     devolve tudo como antes — a Composição e o fluxo antigo dependem disso.
--     Item sem periodo_inicio (não deveria existir; o gerador sempre grava)
--     cai no mês de created_at, para nunca sumir de todas as abas.
--
-- A assinatura antiga (6 parâmetros) é REMOVIDA antes de criar a nova: duas
-- assinaturas com o mesmo nome deixam a chamada ambígua quando o último
-- parâmetro é omitido (mesma armadilha de get_notas_geradas, 20260921130000).
-- A edge get-revisao-fatura chama por nome, então continua funcionando.
-- =====================================================================

-- 1. Escopo de visibilidade do faturamento, compartilhado.
CREATE OR REPLACE FUNCTION finance.escopo_faturamento(p_user_id uuid)
 RETURNS TABLE(tenant_id uuid, can_view_all boolean, viewer_area_id uuid)
 LANGUAGE plpgsql
 STABLE
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF v_tenant_id IS NULL THEN
    -- Quem chama decide a mensagem (get_revisao_fatura sempre levantou
    -- 'Usuário não associado a tenant'); aqui só devolve vazio.
    RETURN;
  END IF;
  ...
  v_can_view_all :=
    ...
    -- Liberacao NOMINAL, para quem precisa ver o escritorio inteiro sem estar
    -- na area VLMA/Financeiro (Filipe, 03/09, sobre a Bruna Bogucheski).
    --
    -- Le a concessao NOMINAL (core.user_permissions), nao
    -- get_user_permissions: essa funcao devolve TODAS as permissoes para quem
    -- e socio, entao qualquer chave nova valeria para Tiago, Victor, Mariana e
    -- Leonardo automaticamente — e o escopo por area, que e proposital, cairia
    -- junto (20260903211000).
    OR EXISTS (
      SELECT 1
      FROM core.user_permissions up
      JOIN core.permissions perm ON perm.id = up.permission_id
      WHERE up.user_id = p_user_id
        AND up.tenant_id = v_tenant_id
        AND perm.chave = 'finance.faturamento.ver_todos'
    );

  tenant_id := v_tenant_id;
  can_view_all := v_can_view_all;
  viewer_area_id := v_viewer_area_id;
  RETURN NEXT;
END;
$function$;

REVOKE ALL ON FUNCTION finance.escopo_faturamento(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION finance.escopo_faturamento(uuid) TO authenticated, service_role;

-- 2. get_revisao_fatura com competência. Corpo idêntico ao de 20260903211000,
--    exceto: escopo via finance.escopo_faturamento, três campos novos por
--    item e o filtro opcional por mês.
DROP FUNCTION IF EXISTS public.get_revisao_fatura(uuid, character varying, text, text, text, text);
...
REVOKE ALL ON FUNCTION public.get_revisao_fatura(uuid, character varying, text, text, text, text, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_revisao_fatura(uuid, character varying, text, text, text, text, date) TO authenticated, service_role;
