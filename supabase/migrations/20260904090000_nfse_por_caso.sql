-- =====================================================================
-- Emissao de NFS-e por CASO (com opcao de agrupar por contrato)
--
-- Filipe, 03/09: "uma nota por caso - e nao por contrato. Um contrato pode ter
-- mais de um caso e o cliente quer receber especificamente o detalhamento na
-- nota por escopo." E pediu flexibilidade para agrupar quando quiser.
--
-- COMO ESTAVA: a emissao nasceu por contrato em 19/05 e nunca foi por caso.
-- Procurei no historico e nao ha registro do escritorio pedindo por contrato —
-- foi escolha de implementacao nossa. Em 28/07 o botao 'Faturar' foi colocado
-- na faixa do CASO mas continuou emitindo o CONTRATO inteiro, sem aviso: e o
-- que o confundiu.
--
-- p_caso_id opcional: nulo mantem o comportamento antigo (contrato inteiro),
-- para nao quebrar chamada existente. Carteira de processos continua gerando
-- uma nota so — ela E um caso unico no sistema, e a trava de
-- 'parte_de_carteira_id IS NULL' impede o processo de dentro de cobrar junto.
-- =====================================================================
...
-- A versao de 2 argumentos precisa SAIR: com a nova tendo p_caso_id DEFAULT,
-- as duas convivem e o Postgres recusa a chamada por ambiguidade
-- ("function is not unique"). Mesma armadilha de sobrecarga que evitamos em
-- cp_editar_lancamento passando o escopo dentro do payload.
DROP FUNCTION IF EXISTS public.get_billing_items_aprovados_full(uuid, uuid);
