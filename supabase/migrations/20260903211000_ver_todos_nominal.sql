-- =====================================================================
-- Corrige a liberacao de 20260903210000: ler a concessao NOMINAL
--
-- A versao anterior checava get_user_permissions, que devolve TODAS as
-- permissoes para quem tem categoria 'socio'. Resultado: a chave nova valia
-- para Tiago, Victor, Mariana e Leonardo tambem, e o escopo por centro de custo
-- — que e proposital — caiu junto. Ficou assim por dez minutos, ate eu medir.
--
-- Agora le core.user_permissions direto: so vale para quem recebeu de fato.
-- =====================================================================
...
BEGIN
  ...
  v_can_view_all :=
    ...
    -- na area VLMA/Financeiro (Filipe, 03/09, sobre a Bruna Bogucheski: "ela
    -- esta vendo o faturamento so dela, pode liberar geral").
    --
    -- Chave propria, e nao finance.faturamento.*: TODO socio de area tem essas
    -- permissoes, entao usa-las aqui liberaria Tiago, Victor, Mariana e
    -- Leonardo de uma vez — o oposto do que o escopo por centro de custo quer.
    -- Mudar a AREA dela resolveria a visao, mas mexeria no centro de custo e
    -- sairia errado em relatorio e rateio.
    OR EXISTS (
      ...
      -- junto. Foi o que aconteceu na primeira versao desta migracao, por dez
      -- minutos, ate eu conferir.
      SELECT 1
