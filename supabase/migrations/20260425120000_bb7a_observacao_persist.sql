-- BB-7a: persistir e ler `observacao` em contracts.casos via RPCs.
-- Coluna existe desde RF-062 e o frontend/edges sempre enviaram/leram o campo,
-- mas as RPCs `public.create_caso` / `public.update_caso` / `public.get_contrato`
-- ignoravam silenciosamente (INSERT sem observacao, SET sem observacao,
-- jsonb_agg do get_contrato sem observacao). Mesma classe do BB-9.
--
-- Padrao: CREATE OR REPLACE preservando assinaturas bit-a-bit do dump atual,
-- aplicando apenas 3 deltas aditivos:
--  1) create_caso : +coluna `observacao` no INSERT (apos `nome`) + valor NULLIF
--  2) update_caso : +linha `observacao = CASE ... END` no SET (apos `nome`)
--  3) get_contrato: +par `'observacao', cs.observacao` no jsonb_build_object da agg de casos
--
-- Idempotente e backward-compat (ADR-008): sem DROP, sem overload novo.
...
BEGIN
  ...
  INSERT INTO contracts.casos (
    tenant_id, contrato_id, nome, observacao, servico_id, produto_id, responsavel_id,
    ...
    v_tenant_id, p_contrato_id, p_payload->>'nome',
...
BEGIN
  ...
  SELECT cs.regras_financeiras, cs.polo INTO v_old_regras, v_old_polo
