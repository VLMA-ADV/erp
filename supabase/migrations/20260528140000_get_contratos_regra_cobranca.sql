-- Adiciona regra_cobranca aos casos retornados por get_contratos.
-- Necessário para o frontend esconder a matriz de carteira mesmo quando ela
-- ainda não tem filhos (matriz vazia recém-criada). O filtro por
-- processos_carteira_count > 0 não cobre esse caso.
--
-- Motivação: Filipe na call 28/05 viu o caso pai (matriz) ainda aparecendo
-- "Curitiba tá aparecendo ainda". A matriz vazia precisa ser identificada
-- por regra_cobranca = 'mensalidade_carteira' AND parte_de_carteira_id IS NULL.
