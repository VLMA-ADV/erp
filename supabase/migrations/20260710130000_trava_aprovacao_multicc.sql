-- B2: trava de aprovação em casos multi-CC (revisores_modo=auto_centro_custo):
-- a aprovação final só libera quando NENHUMA hora do caso estiver em revisão
-- (todos os coordenadores de CC concluíram). Regra da call de 08/07.
...
BEGIN
  ...
  IF v_action = 'avancar' THEN
    ...
    ELSIF v_old_status = 'em_aprovacao' THEN
      ...
      -- Trava multi-CC: em casos com revisão automática por centro de custo,
      -- a aprovação final só libera quando todas as horas do caso saírem da revisão.
      IF v_item.origem_tipo = 'timesheet'
        AND EXISTS (
          SELECT 1 FROM contracts.casos cs
          WHERE cs.id = v_item.caso_id
            AND cs.timesheet_config->>'revisores_modo' = 'auto_centro_custo'
        )
        AND EXISTS (
          SELECT 1 FROM finance.billing_items s
          WHERE s.tenant_id = v_tenant_id
            AND s.caso_id = v_item.caso_id
            AND s.origem_tipo = 'timesheet'
            AND s.status = 'em_revisao'
            AND s.id <> v_item.id
        ) THEN
        RAISE EXCEPTION 'Aprovação bloqueada: ainda há horas deste caso aguardando revisão dos coordenadores de centro de custo.';
      END IF;
